...
module.exports.recipes_ingredients_get = async (
	_req: Request,
	res: Response
) => {
	try {
...
module.exports.recipes_filter_recipes_get = async (
	req: any,
	res: Response
) => {
	const query: Recipe = req.query
	try {
		let where = []
		...
		const sqlQuery =
			'SELECT course, cuisine, prep_time, cook_time, uname, uid, instructions, image_url, rname FROM RECIPE JOIN USERS ON uid=slug ' +
...
module.exports.recipes_single_delete = async (req: any, res: Response) => {
...
module.exports.recipes_single_post = async (req: any, res: Response) => {
	const {
		rname,
		cuisine,
		course,
...
$5, $6, $7, $8) RETURNING *`
		...
		ingredients.forEach(async (ingredient) => {
			await pool.query(
				'INSERT INTO RECIPE_USES_INGREDIENT(iname, amount, uid, rname) VALUES($1,$2,$3,$4)',
				[ingredient.ingredient, ingredient.amount, uid, rname]
			)
		})
		console.log(rows)
